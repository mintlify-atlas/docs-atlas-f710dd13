...
description: "Build APK packages for x86_64, aarch64, armv7, ppc64le, s390x, and more using QEMU emulation — no cross-compilation required."
---
...
<CardGroup cols={3}>
  ...
  <Card title="docker" icon="docker">
    **Linux and macOS.** Runs pipeline steps inside a Docker container. Requires Docker Desktop or a Docker daemon. Useful when bubblewrap is unavailable or Docker is already part of your workflow.
  </Card>
  <Card title="qemu" icon="microchip">
    **Full VM.** Boots a QEMU virtual machine for each build. Required for strict isolation or when `binfmt_misc` is not available. Slower than bubblewrap but most portable. The default on macOS hosts.
  </Card>
</CardGroup>
