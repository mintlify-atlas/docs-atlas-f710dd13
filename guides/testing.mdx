---
title: "Testing APK Packages with melange's test Command"
...
Test pipelines support `uses:` steps just like build pipelines. Point `--pipeline-dir` at a directory containing custom pipeline YAML files:

```shell
melange test ./mypackage-test.yaml --pipeline-dir ./pipelines
```
...
| `--pipeline-dir` | Directory with custom pipeline definitions. |
