---
title: "Get started with melange: Build your first APK"
...
<Note>
  The `--privileged` flag is required when running inside Docker because melange uses [bubblewrap](https://github.com/containers/bubblewrap) to create the isolated build guest, which requires elevated capabilities. When running melange natively outside a container, Docker or Podman must still be available on the host.
</Note>
