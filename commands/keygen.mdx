...
The positional argument sets the output filename for the private key. The public key is written to the same path with `.pub` appended. When omitted, the default filename `local-melange.rsa` is used.

## Examples

```shell
# Generate a key pair with the default filename (local-melange.rsa)
melange keygen
